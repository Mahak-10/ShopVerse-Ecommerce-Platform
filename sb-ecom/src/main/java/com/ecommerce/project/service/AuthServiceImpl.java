...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public ResponseEntity<MessageResponse> updateSeller(Long sellerId, com.ecommerce.project.security.request.SellerUpdateRequest updateRequest) {
        User user = userRepository.findById(sellerId)
                .orElseThrow(() -> new RuntimeException("Error: Seller not found with id: " + sellerId));
        ...
        if (updateRequest.getPassword() != null && !updateRequest.getPassword().trim().isEmpty()) {
            if (updateRequest.getPassword().length() < 6 || updateRequest.getPassword().length() > 40) {
                return ResponseEntity.badRequest().body(new MessageResponse("Error: Password size must be between 6 and 40 characters!"));
            }
            user.setPassword(encoder.encode(updateRequest.getPassword()));
        }

        userRepository.save(user);
        return ResponseEntity.ok(new MessageResponse("Seller updated successfully!"));
    }
}
